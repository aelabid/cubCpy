...
/*   Updated: 2023/01/14 08:01:28 by aelabid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	texture[0].img = mlx_xpm_file_to_image(mlx.mlx_ptr, data.no,
	...
	texture[0].addr = (int *)mlx_get_data_addr(texture[0].img,
			...
			&texture[0].endian);
	texture[1].img = mlx_xpm_file_to_image(mlx.mlx_ptr, data.so,
	...
	texture[1].addr = (int *)mlx_get_data_addr(texture[1].img,
			...
			&texture[1].endian);
	texture[2].img = mlx_xpm_file_to_image(mlx.mlx_ptr, data.ea,
	...
	texture[2].addr = (int *)mlx_get_data_addr(texture[2].img,
			...
			&texture[2].endian);
	texture[3].img = mlx_xpm_file_to_image(mlx.mlx_ptr, data.we,
